...
def set_mode(mode):
    ...
    tips = {
        'VIEW': '【查看/编辑】点击对象可选中。在左侧属性栏修改名称或类型。',
        'ADD_COMP': '【画框模式】拖拽画框，创建新组件。',
        'ADD_PORT': '【端口模式】点击添加端口 (框内自动归属)。',
        'CONNECT': '【连线模式】点击端口A -> 端口B (或点击连接线)。'
    }
    if app_state["ui"]["status"]:
        app_state["ui"]["status"].set_text(tips.get(mode, ''))
...
def on_component_rename(new_val):
    ...
    if success:
        sel["name"] = new_val # 更新选中状态
        ui.notify(f"重命名为: {new_val}")
        refresh_canvas()
    else:
        ui.notify(f"重命名失败: {msg}", color='negative')
        # 重置回原名 (比较复杂，这里简单刷新面板)
...
def update_info_panel(hit):
    ...
    with panel:
        ...
        if hit["type"] == "component":
            # 1. 组件名 (Input)
            ui.input('组件名称 (Name)', value=hit["name"], 
                     on_change=lambda e: on_component_rename(e.value)).classes('w-full')
            
            # 2. 组件类型 (Input)
            curr_type = viz.data["components"][hit["name"]].get("type", "")
            ui.input('组件类型 (Type)', value=curr_type,
                     on_change=lambda e: on_component_type_change(e.value)).classes('w-full')
            
            ui.label(f"包含端口数: {len(viz.data['components'][hit['name']]['ports'])}").classes('text-xs text-gray-500 mt-1')

        elif hit["type"] == "port":
            # 1. 端口名 (Input)
            ui.input('端口名称', value=hit["port"],
                     on_change=lambda e: on_port_rename(e.value)).classes('w-full')
            
            ui.label(f"所属组件: {hit['comp']}").classes('text-sm text-gray-600 mt-2')

        elif hit["type"] == "conn_center":
            ui.label("连接网络 (Connection)").classes('text-lg text-green-600 font-bold')
            node_count = len(viz.data['connections'][hit['index']]['nodes'])
            ui.label(f"包含节点: {node_count}").classes('text-sm text-gray-600')
...
def refresh_canvas(draw_only_temp=False):
    ...
    if not draw_only_temp and app_state["viz"]:
        ...
        for comp in reversed(sorted_comps):
            ...
            if dim_mode:
                ...
                if is_target:
                    stroke = "red"; sw = 4; fill_opacity = 0
                else:
                    stroke = "rgba(0,0,255,0.3)"; fill_opacity = 0.02

            svg_content += f'<rect x="{bx}" y="{by}" width="{bw}" height="{bh}" ' \
                           f'fill="rgba(0,0,255,{fill_opacity})" stroke="{stroke}" stroke-width="{sw}" />'
        ...
        # 2. 连线 (现在有hit_test的线段检测支持，画粗一点方便点选)
        for idx, conn in enumerate(viz.data["connections"]):
            ...
            is_high = False
            if sel:
                if sel["type"] == "conn_center" and sel["index"] == idx: is_high = True
                if sel["type"] == "port": 
            ...
            width = 3 # 稍微加粗
            if dim_mode:
                if is_high: color = "red"; width = 5
                else: color = "rgba(0,200,0,0.2)"
            
            svg_content += f'<circle cx="{center[0]}" cy="{center[1]}" r="{6 if is_high else 4}" fill="{color}" stroke="white" stroke-width="1" />'
            for node in conn["nodes"]:
                p_coord = viz.get_port_coord(node["component"], node["port"])
                if p_coord:
                    svg_content += f'<line x1="{p_coord[0]}" y1="{p_coord[1]}" x2="{center[0]}" y2="{center[1]}" ' \
    ...
    if app_state["temp_draw"]:
        s = app_state["temp_draw"]['start']
        c = app_state["temp_draw"]['curr']
...
async def open_add_comp_dialog(box):
    with ui.dialog() as dialog, ui.card().classes('min-w-[300px]'):
        ...
        name_input = ui.input('名称 (Name)').props('autofocus')
        type_input = ui.input('类型 (Type)')
...
async def open_add_port_dialog(comp_name, coord):
    with ui.dialog() as dialog, ui.card().classes('min-w-[300px]'):
        ui.label(f'新增端口 (归属: {comp_name})').classes('text-lg font-bold')
...
async def handle_mouse(e: events.MouseEventArguments):
    ...
    if mode == 'ADD_COMP':
        if e.type == 'mousedown':
            app_state["temp_draw"] = {'start': (x, y), 'curr': (x, y)}
            ...
            if abs(box[2]-box[0]) > 5 and abs(box[3]-box[1]) > 5:
                await open_add_comp_dialog(box)
            else:
                ui.notify("框太小", color='warning')
        ...
        if mode == 'VIEW':
            ...
            target = "external"
            if comp_hit and comp_hit["type"] == "component": target = comp_hit["name"]
    ...
    with ui.row().classes('w-full h-[calc(100vh-3.5rem)] no-wrap gap-0'):
        ...
        with ui.column().classes('w-72 h-full bg-white border-r border-gray-300 p-4 gap-4 shadow-md shrink-0 z-20'):
            with ui.card().classes('w-full p-2 bg-slate-50'):
                ...
                ui.upload(label='上传图片', on_upload=handle_image_upload, auto_upload=True).props('flat dense bordered color=primary').classes('w-full mb-2')
                ui.upload(label='上传 JSON', on_upload=handle_json_upload, auto_upload=True).props('flat dense bordered color=secondary').classes('w-full')

            with ui.card().classes('w-full p-2 bg-slate-50 gap-2'):
                ui.label('2. 标注模式').classes('font-bold text-xs text-slate-500 mb-1')
                btns = app_state["ui"]["mode_btns"]
        ...
        with ui.column().classes('flex-grow h-full bg-gray-200 relative overflow-auto items-start justify-start p-4'):
            img = ui.interactive_image(
                ...
                cross=True
            ).classes('shadow-2xl bg-white').style('width: 100%; height: auto;')
