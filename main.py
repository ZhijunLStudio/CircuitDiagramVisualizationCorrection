...
def save_history():
    ...
    app_state["history"].append(app_state["viz"].clone_data())
    if len(app_state["history"]) > 20: app_state["history"].pop(0)
    update_undo_btn()

def undo():
    if not app_state["history"]: return
    prev_data = app_state["history"].pop()
    app_state["viz"].restore_data(prev_data)
...
def update_undo_btn():
    btn = app_state["ui"]["undo_btn"]
    if btn:
        if app_state["history"]: btn.enable()
        else: btn.disable()
...
def set_zoom(val):
    app_state["zoom"] = val
    if app_state["ui"]["img"]:
        app_state["ui"]["img"].style(f'transform: scale({val}); transform-origin: top left;') # 左上角对齐缩放

def zoom_in(): set_zoom(min(app_state["zoom"] + 0.1, 3.0))
def zoom_out(): set_zoom(max(app_state["zoom"] - 0.1, 0.2))
...
def set_mode(mode):
    ...
    tips = {
        'VIEW': '【查看/编辑】点击对象选中。选组件/端口高亮整个网络；选连线高亮单根。',
...
def handle_image_upload(e: events.UploadEventArguments):
    ...
    except: ui.notify("图片无法解析", color='negative'); return
...
def handle_json_upload(e: events.UploadEventArguments):
    try:
        ...
        set_mode('VIEW')
    except Exception as ex: ui.notify(f"JSON 错误: {ex}", color='negative')
...
def on_component_rename(new_val):
    ...
    save_history()
    success, msg = app_state["viz"].rename_component(sel["name"], new_val)
    if success: sel["name"] = new_val; ui.notify("重命名成功"); refresh_canvas()
...
def on_component_type_change(new_val):
    ...
    if not sel: return
    save_history()
    app_state["viz"].update_component_type(sel["name"], new_val)

def on_port_rename(new_val):
    sel = app_state["selected"]
    if not sel or sel["port"] == new_val: return
    save_history()
    success, msg = app_state["viz"].rename_port(sel["comp"], sel["port"], new_val)
    if success: sel["port"] = new_val; ui.notify("重命名成功"); refresh_canvas()
    else: ui.notify(msg, color='negative')
...
def update_info_panel(hit):
    ...
    with panel:
        ...
        if hit["type"] == "component":
            ...
            ui.button('删除 整个网络', on_click=delete_selection, color='red').classes('w-full mt-2')
            return
        elif hit["type"] == "conn_edge":
            ui.label("连线分支 (单根)").classes('text-lg text-green-600 font-bold')
            node = hit["node"]
            ui.label(f"{node['component']} -> {node['port']}").classes('text-sm text-gray-700 my-2')
            ui.button('断开 此连线', on_click=delete_selection, color='orange').classes('w-full mt-2')
            return
...
def refresh_canvas(draw_only_temp=False):
    ...
    if not draw_only_temp and app_state["viz"]:
        ...
        for idx, conn in enumerate(viz.data["connections"]):
            ...
            # --- 判定逻辑 ---
            # network_high: 是否高亮整个网络
            network_high = False
            
            if sel:
                # 1. 选中中心 -> 高亮全网
                if sel["type"] == "conn_center" and sel["index"] == idx:
                    network_high = True
                # 2. 选中组件 -> 高亮与该组件相关的所有网络
                elif sel["type"] == "component":
                    for node in conn["nodes"]:
                        if node["component"] == sel["name"]:
                            network_high = True; break
                # 3. 选中端口 -> 高亮与该端口相关的所有网络
                elif sel["type"] == "port":
                     for node in conn["nodes"]:
                        if node["component"] == sel["comp"] and node["port"] == sel["port"]:
                            network_high = True; break
            
            # 绘制中心
            c_color = "red" if network_high else ("#00cc00" if not dim_mode else "rgba(0,200,0,0.2)")
        ...
        for p in all_ports:
            cx, cy = p["coord"]
            is_ext = (p["type"] == "ext")
            r = 10 if is_ext else 5 # 外部端口画大一点
            
            p_high = False
            if sel and sel["type"] == "port" and sel["comp"] == p["comp"] and sel["port"] == p["name"]: p_high = True
            if app_state["connect_start"] and app_state["connect_start"]["comp"] == p["comp"] and app_state["connect_start"]["port"] == p["name"]: p_high = True
            ...
            if not app_state["connect_start"]:
                ...
                if hit["type"] == "port":
                    if start != hit:
                        ...
                        ui.notify("连接成功"); app_state["connect_start"] = None; refresh_canvas()
                elif hit["type"] in ["conn_center", "conn_edge"]:
                    viz.add_to_connection(hit["index"], start)
                    ui.notify("已合并"); app_state["connect_start"] = None; refresh_canvas()
...
def main():
    ...
    with ui.header().classes('bg-slate-800 items-center h-14 shadow-lg'):
        ...
        # 撤销
        app_state["ui"]["undo_btn"] = ui.button('撤销', icon='undo', on_click=undo).props('flat color=white').tooltip('Ctrl+Z')
        app_state["ui"]["undo_btn"].disable()
    ...
    with ui.row().classes('w-full h-[calc(100vh-3.5rem)] no-wrap gap-0'):
        ...
        # 右侧画布区：恢复 overflow-auto, 允许滚动
        with ui.column().classes('flex-grow h-full bg-gray-500 relative overflow-auto items-start justify-start'):
            # 恢复 width: 100%, height: auto
            img = ui.interactive_image(
                events=['mousedown', 'mouseup', 'mousemove'],
                on_mouse=handle_mouse,
                cross=True
            ).style('width: 100%; height: auto; transform-origin: top left; transition: transform 0.1s ease-out;')
            
            app_state["ui"]["img"] = img

            # 缩放控件
            with ui.column().classes('fixed bottom-4 right-4 gap-2 z-50'):
                ui.button(icon='add', on_click=zoom_in).props('round color=white text-color=black shadow')
                ui.button(icon='restart_alt', on_click=zoom_reset).props('round color=white text-color=black shadow').tooltip('重置 1:1')
                ui.button(icon='remove', on_click=zoom_out).props('round color=white text-color=black shadow')
